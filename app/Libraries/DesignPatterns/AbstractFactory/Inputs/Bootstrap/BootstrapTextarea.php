...
/**
 * Class BootstrapTextarea
 * @package App\Libraries\DesignPatterns\AbstractFactory\Inputs\Bootstrap
 */
class BootstrapTextarea implements InputInterface
{
    ...
    public function render($return = false): string
    {
        ...
        $html .= '<label for="' . $this->getName() . '">This is Bootstrap Textarea:</label>';
        $html .= '<textarea type="' . $this->getType() . '" class="form-control ' . $this->getClass() . '" id="' . $this->getId() . '" placeholder="' . $this->getName() . '"></textarea>';
        $html .= '</div>';
        if (!$return) {
            echo $html;
        }
